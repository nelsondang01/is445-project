...
app.post("/customers/search", async (req, res) => {
  const { cusId, cusFname, cusLname, cusState, cusSalesYTD, cusSalesPrev } = req.body;
  ...
  res.render("customers", {
    ...
    count: countResult.rows[0].count,
    message: customersResult.rows.length === 0 ? "No records were found." : "",
    search: req.body
  });
});

app.get("/customers/edit/:id", async (req, res) => {
  const result = await pool.query("SELECT * FROM customer WHERE cusId = $1", [req.params.id]);
...
app.post("/customers/edit/:id", async (req, res) => {
  const { cusFname, cusLname, cusState, cusSalesYTD, cusSalesPrev } = req.body;

  if (!cusFname || !cusLname) {
    return res.render("editCustomer", {
      customer: { cusid: req.params.id, cusfname: cusFname, cuslname: cusLname, cusstate: cusState, cussalesytd: cusSalesYTD, cussalesprev: cusSalesPrev },
  ...
  await pool.query(
    `UPDATE customer
     ...
     WHERE cusId = $6`,
    [cusFname, cusLname, cusState, cusSalesYTD || 0, cusSalesPrev || 0, req.params.id]
  );
...
app.post("/import", upload.single("customerFile"), async (req, res) => {
  ...
  for (let line of lines) {
    ...
    const [cusId, cusFname, cusLname, cusState, cusSalesYTD, cusSalesPrev] = line.split(",");
    ...
    try {
      ...
      notInserted++;
      errors.push(`Customer ID ${cusId}: ${err.message}`);
    }
  }

  fs.unlinkSync(req.file.path);

  const newCountResult = await pool.query("SELECT COUNT(*) FROM customer");
